...
Before running these commands on a fresh clone:
...
The repository validates the Node version during `yarn install` and again in `setup-dev-env.sh`, so fix any toolchain mismatch before running Nx commands.
